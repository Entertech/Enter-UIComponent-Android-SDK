...
public class HexDump {
    ...
    public static String getValidateVode(byte[] bytes) {
        if (bytes == null || bytes.length == 0) {
            return "0000";
        }
        int count = 0;
        for (int i = 0; i < bytes.length; i++) {
            count = count + (int) bytes[i];
        }
    ...
    public static byte[] floatArrayToByteArray(float[] data) {
        ...
        for (int i = 0; i < data.length; i++) {
            byte[] intToBytes2 = toByteArray(Float.floatToIntBits(data[i]));
            byte[] temp = new byte[4];
            temp[0] = intToBytes2[3];
            temp[1] = intToBytes2[2];
            temp[2] = intToBytes2[1];
            temp[3] = intToBytes2[0];
            Resutl = mergeByteArrays(Resutl, temp);
        }
        return Resutl;
    }
     ...
     * @return
     */
    public static byte[] mergeByteArrays(byte[] first, byte[]... rest) {
